(() => {
  ...
  // ---- T&S audio (aggr-style synth tones) ----
  const TS_AUDIO = { engine: null, ready: false };
  class AggrToneEngine {
    constructor() {
      this.ctx = null;
      this.master = null;
      this.comp = null;
    ...
    _nextTime(delaySec = 0) {
      if (!this.ctx) return 0;
      let cueTime = 0;
      if (!this.minTime) {
        this.minTime = this.ctx.currentTime;
      } else {
        this.minTime = Math.max(this.minTime, this.ctx.currentTime);
        if (!delaySec && this.count) {
          cueTime = this.count > 10 ? (this.count > 20 ? (this.count > 100 ? 0.01 : 0.02) : 0.04) : 0.08;
        }
      }
      if (!cueTime && !delaySec) cueTime = 0.08;
      const t = this.minTime + cueTime + Math.max(0, delaySec || 0);
      this.minTime += cueTime;
      this.count++;
      return t;
    }

    _playNote(freq, gain, fadeOut = 0.18, delay = 0, oscType = 'sine') {
      if (globalSilent || !this.ctx || this.ctx.state !== 'running') return;
      const targetTime = this._nextTime(delay);
      const waitMs = Math.max(0, (targetTime - this.ctx.currentTime) * 1000);
      setTimeout(() => {
        if (!this.ctx || !this.master || this.ctx.state !== 'running') return;
        const now = this.ctx.currentTime;
        const osc = this.ctx.createOscillator();
        const g = this.ctx.createGain();
        osc.frequency.value = Math.max(40, +freq || 440);
        osc.type = oscType;

        const startGain = 0.0001;
        const endGain = 0.0001;
        const targetGain = Math.max(0.0002, +gain || 0.02);
        const fo = Math.max(0.05, +fadeOut || 0.18);

        g.gain.setValueAtTime(startGain, now);
        g.gain.exponentialRampToValueAtTime(targetGain, now + 0.003);
        g.gain.exponentialRampToValueAtTime(endGain, now + fo);

        osc.connect(g);
        g.connect(this.master);
        osc.start(now);
        osc.stop(now + fo + 0.002);
        osc.onended = () => {
    ...
    playTrade(ev, intensity, heavyVoices) {
      if (globalSilent || !this.ctx || this.ctx.state !== 'running') return;
      const side = (ev && ev.side) || '';
      const isBuy = side === 'at_ask' || side === 'above_ask' || side === 'between_ask' || side === 'between_mid';
      const baseNotes = isBuy ? [659.26, 830.6, 987.76, 1318.52] : [493.88, 392.0, 293.66, 246.94];
      const I = Math.max(0, Math.min(1, Number.isFinite(intensity) ? intensity : 0));
      const ratio = Math.max(0.08, Math.min(1.3, Math.sqrt(0.05 + I)));

      let voices = 1;
      if (heavyVoices) {
        if (I >= 0.85 || (ev && ev.big)) voices = 4;
        else if (I >= 0.4) voices = 2;
      }

      const perVoiceGain = Math.max(0.02, Math.min(0.22, (0.05 + ratio / (voices === 1 ? 10 : 16)) * (isBuy ? 1.0 : 1.15)));
      const fadeOut = voices === 1 ? (0.1 + ratio / 7) : (0.12 + ratio * 0.22);
      for (let i = 0; i < voices; i++) {
        this._playNote(baseNotes[i], perVoiceGain, fadeOut, i * 0.08, 'sine');
      }
    }

    tick({ f0 = 1000, f1 = 900, gain = 0.05, dur = 0.018 } = {}) {
      if (globalSilent || !this.ctx || this.ctx.state !== 'running') return;
  ...
  function tsRowHtml(ev) {
    const colorClass = {
      above_ask: 'col-yellow',
      at_ask: 'col-green',
      between_mid: 'col-white',
      between_ask: 'col-white',
      between_bid: 'col-white',
      at_bid: 'col-red',
      below_bid: 'col-magenta'
    }[ev.side] || 'col-white';
    ...
    const sideLabel = escapeHtml((ev.side || '').replaceAll('_', ' '));
    const sym = escapeHtml(ev.sym || activeSymbol || '');
    const amountStr = escapeHtml(ev.amountStr || '');
    return `<div class="row${ev.big ? ' big' : ''}">
      <div class="left">
        <span class="badge ${colorClass}">${sideLabel}</span>
        <span class="price ${colorClass}">${priceStr}</span>
        <span class="amt">$${amountStr} <span class="shares">(${sharesStr})</span></span>
      </div>
  ...
  function prependTapeRows(rows) {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      html += tsRowHtml(rows[i]);
    }
    c.insertAdjacentHTML('afterbegin', html);

    const over = c.childElementCount - 1000;
    if (over > 0) {
      for (let i = 0; i < over; i++) c.removeChild(c.lastElementChild);
    }

    const newScrollHeight = c.scrollHeight;
    c.scrollTop = atTop ? 0 : (prevScrollTop + (newScrollHeight - prevScrollHeight));
  }
  ...
  function tradeIntensity(ev) {
    const amt = Number(ev && ev.amount) || 0;
    if (!Number.isFinite(amt) || amt <= 0) return 0;
    const lo = Math.max(1, Number(tns.dollar || 1));
    const hi = Math.max(lo * 20, Number(tns.bigDollar || (lo * 50)));
    const denom = Math.log(hi / lo);
    if (!Number.isFinite(denom) || denom <= 0) return 0;
    const x = Math.log(amt / lo) / denom;
    return clamp01(x);
  }

  function scheduleTapeFlush() {
    if (tapeFlushScheduled) return;
    tapeFlushScheduled = true;
    setTimeout(flushTSTrades, 0);
  }

  function flushTSTrades() {
    tapeFlushScheduled = false;
    if (!tapeTradeQueue.length) return;
    const batch = tapeTradeQueue.splice(0, TAPE_RENDER_BATCH_MAX);
    if (tapeTradeQueue.length) scheduleTapeFlush();

    if (els.tape) prependTapeRows(batch);

    const lastEv = batch[batch.length - 1];
    if (els.vol && lastEv && lastEv.volume != null) {
      els.vol.textContent = formatVolumeK(lastEv.volume);
    }
    if (lastEv) {
      onMarketPulse({
        price: lastEv.price,
        last: lastEv.last,
        volume: lastEv.volume,
        timeISO: lastEv.timeISO
      });
    }

    for (let i = 0; i < batch.length; i++) {
      updateOrderflowFromTrade(batch[i]);
    }

    if (globalSilent || !TS_AUDIO.ready || !TS_AUDIO.engine || !tsMachineGunClicks) return;
    for (let i = 0; i < batch.length; i++) {
      const ev = batch[i];
      const I = tradeIntensity(ev);
      const heavyVoices = !tsVoiceBigOnly || !!ev.big || I > 0.7;
      TS_AUDIO.engine.playTrade(ev, I, heavyVoices);
    }
  }

  function onTSTrade(ev) {
    if (!ev) return;
    tapeTradeQueue.push(ev);
    if (tapeTradeQueue.length > TAPE_QUEUE_MAX) {
      tapeTradeQueue.splice(0, tapeTradeQueue.length - TAPE_QUEUE_MAX);
    }
    scheduleTapeFlush();
  }

  function onTSTrades(evs) {
