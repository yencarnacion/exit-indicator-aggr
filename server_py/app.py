...
TRADE_BATCH_MAX = max(1, int(os.getenv("EI_TRADE_BATCH_MAX", "256") or "256"))
...
async def _trade_worker():
    try:
        while True:
            ...
            batch = [ev]
            while len(batch) < TRADE_BATCH_MAX:
                try:
                    batch.append(_trade_q.get_nowait())
                except asyncio.QueueEmpty:
                    break
            try:
                await broadcast_trades(batch)
            finally:
                for _ in batch:
                    _trade_q.task_done()
...
def _build_trade_payload(ev: dict) -> dict | None:
    if recorder:
...
async def broadcast_trades(evs: list[dict]):
    payloads: list[dict] = []
    rvol_alerts: list[RVOLAlert] = []
    rvol_enabled = bool(getattr(cfg, "rvol_enabled", True))

    for ev in evs:
        payload = _build_trade_payload(ev)
        if payload is None:
            continue
        payloads.append(payload)
        if rvol_enabled:
            alerts = rvol_manager.on_trade(price=float(payload["price"]), size=int(payload["size"]))
            if alerts:
                rvol_alerts.extend(alerts)

    if payloads:
        if len(payloads) == 1:
            await broadcast(payloads[0])
        else:
            await broadcast({"type": "trades", "data": payloads})

    for alert in rvol_alerts:
        await broadcast_rvol_alert(alert)

async def broadcast_trade(ev: dict):
    await broadcast_trades([ev])
